            ...
            const tripsHtml = trips.map(trip => {
                ...
                return `
                    <div class="trip-item" data-trip-id="${trip.id}">
                        ...
                        <div>${trip.startStop || trip.startStopCode || trip.startStopName || 'Unknown'} → ${trip.endStop || trip.endStopCode || trip.endStopName || 'Unknown'}</div>
                ...
                .then((snapshot) => {
                    if (!snapshot.empty) {
                        ...
                        snapshot.forEach((doc) => {
                            ...
                            const startStop = trip.startStop || trip.startStopCode || trip.startStopName || 'Unknown';
                            const endStop = trip.endStop || trip.endStopCode || trip.endStopName || 'Unknown';
        ...
        // Load unverified SMS trips for review
            ...
            db.collection('trips')
                ...
                .then((snapshot) => {
                    if (!snapshot.empty) {
                        ...
                        unverifiedCount.textContent = `${snapshot.size} unverified`;
                        ...
                        snapshot.forEach((doc) => {
                            ...
                            tripDiv.innerHTML = `
                                <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                    <div>
                                        <div style="font-weight: 600; color: var(--text-primary);">${trip.route} <span class="agency-badge">• ${agency}</span></div>
                                        <div style="font-size: 0.9em; color: var(--text-secondary);">${startStop} → ${endStop}</div>
                                    </div>
                                    <div style="text-align: right;">
                                        <div style="font-size: 0.85em; color: var(--text-muted);">${dateStr}</div>
                                        <button class="btn btn-sm" style="background: var(--accent-primary); color: white; margin-top: 5px;">Add Stop</button>
                                    </div>
                                </div>
                            `;
                            tripDiv.querySelector('button').addEventListener('click', (e) => {
                                e.stopPropagation();
                ...
                .catch((error) => {
                    console.error('Error loading unverified trips:', error);
                    reviewTripsSection.style.display = 'none';
                });
        }
