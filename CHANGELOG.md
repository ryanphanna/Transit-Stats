...
### Fixed
- **Three more training/inference skew bugs fixed** (`functions/lib/ml_utils.js`, `functions/lib/predict_v4.js`, `functions/lib/predict_v5.js`, `ml/train_endstop.py`, `ml/train_routes.py`): (1) `" and "` separator not handled in JS `canonicalizeStop` — stops like "Spadina and Nassau" produced different feature keys in training vs inference; (2) `last_end_stop` defaulted to `"none"` in Python training but JS generated `last_stop_unknown` on first trip of the day — the none class never matched; (3) V4 route model was trained with `transfer_rarity` but inference never set it (always 0); V5 took first match instead of averaging. Retrained as V5.7/V4.7.
...
### Changed
- **Retrained on 573 trips (V5.5/V4.5)**: End-stop V5.5 84.5% top-1 (+14.7pp), V4.5 75.9% (+11.7pp). Route V5.5 81.0%. 38 new trips unlocked by stop library backfill. New 19th end-stop class.
- **Stop library additions**: Added College St at St George St (stop 845, Westbound, route 506); updated College / Beverley (stop 815, Eastbound) with aliases; added Dufferin St at Dufferin Park Ave (stop 2040, Northbound, routes 29/929). Backfilled stop_matched=true on 35 trips.
- **Added direction feature to end-stop model** (`ml/train_endstop.py`, `functions/lib/ml_utils.js`, `functions/lib/predict_v4.js`, `functions/lib/predict_v5.js`): Direction (northbound/southbound/eastbound/westbound) is now a one-hot feature in the V4/V5 end-stop classifier. Root cause of low production accuracy: without direction, Route 1 trips to Spadina Station (southbound) and York University (northbound) were indistinguishable to the model. V5.4 top-1 improves 66.0% → 69.8% on held-out test. `normalizeDirectionForMl` added to `ml_utils.js` and imported by both engines.
- **Retrained end-stop models on 535 trips (V5.4/V4.4)**: Direction-aware models replace V5.4/V4.4-pre-direction. Stop name canonicalization confirmed correct — all but 5 one-off trips covered by stops library aliases.
