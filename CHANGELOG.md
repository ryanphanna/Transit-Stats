...
### Added
- **HabitEngine v1.0** (`functions/lib/habit.js`, `functions/test_habit.js`): New engine that learns recurring trip patterns from history. Groups completed trips by (stop, route, direction, day-of-week, 2-hour bucket) and scores each group with a confidence formula — count × recency decay (30-day half-life) × time-window precision. `match()` returns the best habit above 0.75 confidence for the current boarding context. `habitPrediction` is now stored on every trip document at start time and rebuilt automatically in the background after each trip ends. Wired into `handlers-trip.js` alongside V3/V4/V5 predictions.
