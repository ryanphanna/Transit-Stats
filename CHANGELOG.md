...
### Changed
- **Intelligence docs restructure** (`docs/INTELLIGENCE.md`, `docs/intelligence/V3.md`): Moved the long V3 detailed version history (v1 through v3.3) into a dedicated archive sub-page. Main file now contains only high-level status, active signals table, current config, and a concise pointer for V3 — plus the existing V6 section with links to experiments, spike, and the user-intent philosophy in TRIP_CORRECTIONS.md. This keeps the primary notebook usable and scannable as V6 work grows. (Under the single approved V6 Notion task.)
- **Intelligence top-level page stabilization** (`docs/INTELLIGENCE.md`, `docs/intelligence/V4.md`, `docs/intelligence/V5.md`): Per the rule that top-level pages must not contain information likely to change regularly, extracted all volatile V4/V5 version histories, accuracy benchmarks with trip counts, "still to build" lists, and per-iteration details into dedicated sub-pages. Main file now holds only stable high-level Role/What it does/Status + pointers for V4 and V5. Completes the top-level cleanup started with V3. (Single V6 task.)
...
- **Prediction Engine V6 — initial spike** (`docs/V6_DESIGN_SPIKE.md`): Small analysis of recent accuracy trends + failure modes. Recommended starting with a Journey-Sequence model as the first real V6 direction (leverages new background finalization + journey linking). Design note created under the single approved Notion task.
- **V6 documentation landing** (`docs/INTELLIGENCE.md`): Added proper `### V6` section (matching V3/V4/V5 structure) with link to the spike doc. V6 now appears in the main Prediction Models overview.
- **V6 doc cross-linking** (`docs/INTELLIGENCE.md`, `docs/V6_EXPERIMENTS.md`): Added links between `INTELLIGENCE.md` V6 section and `V6_EXPERIMENTS.md` (plus light reference to `TRIP_CORRECTIONS.md` for the user-intent philosophy). Reciprocal link added in the experiments log.
- **V6 Experiments Log** (`docs/V6_EXPERIMENTS.md`): New dedicated document created for recording V6 experiments in a scientific format (hypothesis, method, results with sample sizes, interpretation). First two entries added. Linked from the main spike doc.
- **Data quality & correction hygiene** (`docs/TRIP_CORRECTIONS.md`, `docs/INTELLIGENCE.md`, `docs/V6_EXPERIMENTS.md`): Made the \"record user intent\" approach more prominent with a new top-level Guiding Philosophy section in TRIP_CORRECTIONS.md, plus explicit cross-references and data quality notes in INTELLIGENCE.md (V6 area) and V6_EXPERIMENTS.md.
- **Trip Corrections — scenarios added** (`docs/TRIP_CORRECTIONS.md`): Added a "Scenarios" subsection with three concrete examples (stop typo on transfer route, route mistype, low-confidence input) explicitly listing every relevant field/flag to set in each case.
- **Trip Corrections — "What to Do If..." guide** (`docs/TRIP_CORRECTIONS.md`): Added a decision-oriented "What to Do If..." subsection covering common situations such as reprocessing corrected trips, handling low-confidence cases after intent recording, multi-field corrections, retrofitting old corrections, and V6 training implications — with explicit field/flag guidance for each.
- **Trip Corrections — detailed field guidance** (`docs/TRIP_CORRECTIONS.md`): Significantly expanded "Recording User Intent in Corrections" into a specific per-field checklist covering `startStop`, `startStopName`, `stop_matched`, `correctedFields`, `originalValues`, `correctedAt`, `needs_reprocess`, `exclude_from_training`, and `exclude_from_accuracy`, with a detailed before/after from the real May 2026 trip.
- **Trip Corrections philosophy** (`docs/TRIP_CORRECTIONS.md`): Significantly expanded "Recording User Intent in Corrections" with a concrete practical pattern (record intended raw input in `startStop`, keep canonical `startStopName`, set `stop_matched` when context supports it, always apply full metadata + exclusion flags) plus a real example from the May 2026 506 trip (`aogaJmAix5dKdLA0ABzP`).
- **V6 Experimental Philosophy** (`docs/V6_EXPERIMENTS.md`): Added short section formalizing the scientific approach to V6 development.
- **V6 sequence audit v1.1**: Additional measurement showed that naively predicting the previous route within a journey (6.5%) is worse than a dumb global baseline (31%). Useful negative result. Documented in the experiments log.
