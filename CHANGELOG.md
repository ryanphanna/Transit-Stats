...
- **Public and signed-in maps now use the same history and dashboard facts**: a public profile no longer shows a reduced trip count or a different card layout.
...
- **Map startup now feels consistent**: logged-in and public maps share the same wider starting view, while saved-coordinate dots appear before detailed stop matching finishes.
...
- **Additional imported stop aliases now use route-verified local GTFS coordinates**: more Hamilton, Niagara Falls, Cleveland, Toronto, Montreal, Portland, Seattle, and Barcelona labels resolve without changing raw trips.
...
- **Beta now has an isolated GTFS route heatmap at `/heatmap`** that colours route corridors by personal ride frequency without changing the main map.
- **Local and beta dashboards now link directly to the route heatmap** so the beta experiment is easy to find.

### Fixed
- **Live reload no longer signs out valid sessions**: transient Firebase restoration and whitelist failures no longer clear a logged-in user.
