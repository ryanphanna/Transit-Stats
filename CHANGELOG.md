...
- **Transfer-complex reasoning + provisional journey hints** (`functions/lib/transfer-connections.js`, `functions/lib/transfer.js`, `functions/lib/handlers-trip.js`, `functions/test_transfer.js`, `functions/test_handlers.js`):
  - **Connected-stop library**: Added a small curated transfer-complex layer, starting with `college_complex`, so separate normalized stops like the `506` surface stop at Yonge and `College Station` can remain distinct records while still counting as one rider-meaningful transfer point.
