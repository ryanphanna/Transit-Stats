...
- [x] **Shared-destination duration inference** — if trips show A→C takes longer than B→C in the same direction, infer A→B as an intermediate reachable stop. This keeps the trips-only graph from excluding valid intermediate destinations just because the rider has not ended there directly from A.
...
- [x] **Chronological NetworkEngine replay for V6 evaluation** — the replay evaluator can rebuild the learned graph from completed trips in time order. Hard filtering regressed V6; soft bucket-local narrowing is now near parity on the TTC SMS sample, so it remains default-off.
- [ ] **NetworkEngine replay promotion analysis** — decide whether trip-learned reachability should become a V6 soft hint after it shows measurable lift. Never read today's full graph for historical trips, and never seed the graph from GTFS/topology.
