...
function stopLabels(line, canon, direction) {
    ...
    return [canon, ...((line.aliases && line.aliases[canon]) || [])]
        .map(normalizeStopLabel)
        .filter(Boolean);
}
...
export function getConstraint(topology, context, options = {}) {
    ...
    for (let i = 0; i < (line.stops || []).length; i++) {
        if (goingHigher ? i > boardingIdx : i < boardingIdx) {
            for (const label of stopLabels(line, line.stops[i], direction)) legalStops.add(label);
        }
    }
...
function stopAllowedByConstraint(stopName, constraint, options = {}) {
    ...
    const labels = [normalizeStopLabel(stopName), options.canonicalizeStop?.(stopName)].filter(Boolean);
    return labels.some(label => constraint.legalStops.has(label));
}

export function filterCandidatesByConstraint(candidates, constraint, options = {}) {
